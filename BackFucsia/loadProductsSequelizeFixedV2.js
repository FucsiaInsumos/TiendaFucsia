...
async function loadProductsWithSequelize() {
  ...
  try {
        ...
        try {
          ...
          if (error.name === "SequelizeUniqueConstraintError") {
            ...
            if (existingCat) {
              ...
              console.log(
                `📁 Categoría existente encontrada: ${categoriaName}`
              );
            }
          } else {
            console.error(
              `❌ Error creando categoría ${categoriaName}:`,
              error.message
            );
          }
        }
    ...
    for (const [subcategoriaName, categoriaParent] of subcategoriasUnicas) {
      ...
      if (
        !categoryHierarchyMap.has(hierarchyKey) &&
        !categoryMap.has(childKey)
      ) {
        ...
        if (parentCategory) {
          try {
            ...
            console.log(
              `📂 Nueva subcategoría creada: ${categoriaParent} -> ${subcategoriaName}`
            );
          } catch (error) {
            if (error.name === "SequelizeUniqueConstraintError") {
              // Si ya existe, buscarla y agregarla al mapa
              const existingSubcat = await Category.findOne({
                where: { name: subcategoriaName },
              });
              if (existingSubcat) {
                categoryMap.set(childKey, existingSubcat);
                categoryHierarchyMap.set(hierarchyKey, existingSubcat);
                console.log(
                  `📂 Subcategoría existente encontrada: ${subcategoriaName}`
                );
              }
            } else {
              console.error(
                `❌ Error creando subcategoría ${subcategoriaName}:`,
                error.message
              );
            }
          }
      ...
      for (const [index, producto] of batch.entries()) {
        try {
          ...
          if (producto.CATEGORIA && producto.SUBCATEGORIA) {
            ...
            console.log(`[DEBUG] Buscando subcategoría: ${hierarchyKey}`);
            if (subcategory) {
              console.log(
                `[DEBUG] Subcategoría encontrada: id=${subcategory.id}, name=${subcategory.name}`
              );
            } else {
              console.log(
                `[DEBUG] Subcategoría NO encontrada para: ${hierarchyKey}`
              );
            }
            ...
            const category = categoryMap.get(producto.CATEGORIA.toLowerCase());
              console.log(`[DEBUG] Buscando categoría principal: ${producto.CATEGORIA.toLowerCase()}`);
  if (category) {
    console.log(`[DEBUG] Categoría principal encontrada: id=${category.id}, name=${category.name}`);
  } else {
    console.log(`[DEBUG] Categoría principal NO encontrada para: ${producto.CATEGORIA.toLowerCase()}`);
  }
            ...
            console.log(`[DEBUG] Buscando subcategoría como principal: ${producto.SUBCATEGORIA.toLowerCase()}`);
  if (category) {
    console.log(`[DEBUG] Subcategoría como principal encontrada: id=${category.id}, name=${category.name}`);
  } else {
    console.log(`[DEBUG] Subcategoría como principal NO encontrada para: ${producto.SUBCATEGORIA.toLowerCase()}`);
  }
  finalCategoryId = category ? category.id : null;
          }
          ...
          const productData = {
            ...
            purchasePrice: parseFloat(
              producto["purchase price"] || producto.purchasePrice || 0
            ),
            price: parseFloat(producto.price || 0),
            distributorPrice: producto.distributorPrice
              ? parseFloat(producto.distributorPrice)
              : null,
            stock: parseInt(producto["Stock actual"] || producto.stock || 0),
            minStock: parseInt(
              producto["Stock mínimo"] || producto.minStock || 5
            ),
            isPromotion:
              producto.isPromotion === "true" || producto.isPromotion === true,
            isFacturable:
              producto.Facturable === "true" ||
              producto.isFacturable === "true" ||
              producto.isFacturable === true,
            promotionPrice: producto["Precio de promoción"]
              ? parseFloat(producto["Precio de promoción"])
              : null,
          ...
          console.error(
            `❌ Error procesando producto ${i + index + 1}: ${error.message}`
          );
          console.error(
            `   SKU: ${producto.sku || "N/A"} | Nombre: ${
              producto.Name || producto.name || "N/A"
            }`
          );
          console.error(
            `   Categoría: ${producto.CATEGORIA || "N/A"} | Subcategoría: ${
          ...
          if (!finalCategoryId) {
            console.error(
              "   ⚠️ Motivo: No se encontró categoryId para este producto. Revisa los nombres de categoría/subcategoría."
            );
          }
        }
      }
